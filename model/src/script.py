...
def update_model():
    ...
    print('Test data has %i rows and %i columns' %
    ...
    for (feature_name, rescale_method) in rescale_features.items():
        ...
        if scale == 0.:
            ...
            print('Feature %s was dropped because it has no variance' %
                  feature_name)
        else:
    ...
    probabilities = pd.DataFrame(
        data=_probas, index=X_test.index, columns=cols)

    # Build scored dataset
    results_test = X_test.join(predictions, how='left')
    results_test = results_test.join(probabilities, how='left')
    results_test = results_test.join(test["__target__"], how='left')
    results_test = results_test.rename(
        columns={'__target__': 'machine_failure'})
